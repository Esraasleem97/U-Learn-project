...
<div class="bg-gray-100 ">
    <form action="{{route('teacher.update' , $lesson->id)}}" method="post"  enctype="multipart/form-data">
        @csrf
        <div class="container mx-auto ">
            ...
            <div class="border shadow-xl rounded-md w-full bg-white px-12 pb-10">
                <div>
                    <h1 class="text-3xl font-semibold my-8">تعديل درس {{$lesson->title}}</h1>
                </div>
                <div class="w-full sm:w-11/12 md:w-3/4 lg:w-2/3 my-16">
                    ...
                    <div class="input-form my-8">
                        <input type="text" id="title" name="title" value="{{$lesson->title}}" placeholder=" " class="input"/>
                        <label for="title" class="label font-bold">العنوان</label>
                    </div>
                    @error('details') <span class="text-red-400">{{$message}}</span> @enderror

                    <div class="input-form mb-8">
                            <textarea name="details"  id="details" cols="30" rows="10" placeholder=" "
                                      class="input">{{$lesson->details}}</textarea>
                        <label for="details" class="label font-bold">الملخص</label>
                    </div>

                    @error('media') <span class="text-red-400">{{$message}}</span> @enderror
                    ...
                    <template id="file-template">
                        <li class="block p-1 w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/6 xl:w-1/8 h-24">
                            <article tabindex="0"
                                ...
                                <img src="{{$lesson->media}}" alt="upload preview"
